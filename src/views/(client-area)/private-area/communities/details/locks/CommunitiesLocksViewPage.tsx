...
import { getCommunityLocksPaginated } from '@/actions/client-portal/community-locks-actions';
...
/**
 * Vista de puertas: cada cerradura con lo que el fabricante dice de ella.
 *
 * **No se configura nada aquí**, y por eso el listado basta: el horario y los métodos con los que se abre
 * son del llavero, no de la puerta. Antes esta pantalla pedía además el horario de cada puerta —una llamada
 * por tarjeta— a una ruta que ya no existe, y ese horario ni siquiera se pintaba.
 *
 * A diferencia del resto de listados del portal, esta pantalla no usa el
 * componente `Table`: cada `LockCard` es una tarjeta rica y no una fila
 * tabular, así que sigue el patrón más simple de paginación/búsqueda por URL
 * (`ClientListSearch` + `ClientListPagination`) en vez de
 * `useClientTableUrlState`. Sin orden: la rejilla de tarjetas no lo necesita.
...
export default async function CommunitiesLocksViewPage({ serviceId, locale, searchParams }: LocksViewPageProps) {
  ...
  return (
    <>
      ...
      {locks.length > 0 ? (
        locks.map((lock) => (
          <LockCard key={lock.id} lock={lock} locale={locale} />
        ))
      ) : (
